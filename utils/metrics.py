...
data20D = ['Weibull_Uniform20D_v1', 'WeibullUniform20D_v1_heavy', 'WeibullUniform20D_v1_light', 'Gamma_Uniform20D_v1', 'GammaUniform20D_v1_heavy', 'GammaUniform20D_v1_light']
synth_tar_list = ['Gaussian_linear', 'Gaussian_nonlinear','Exponential','Weibull','LogNorm','Gaussian_uniform','GaussianUniform4D_v1_heavy','Gaussian_Uniform4D_v1','GaussianUniform4D_v1_light','Gaussian_Uniform4D_v2','LogNorm_v1_heavy','LogNorm_v1','LogNorm_v1_light','LogNorm_v2','GaussianUniform10D_v1','GaussianUniform10D_v1_heavy','GaussianUniform10D_v1_light','Weibull_Uniform10D_v1','WeibullUniform10D_v1_heavy','WeibullUniform10D_v1_light','Gamma_Uniform10D_v1','GammaUniform10D_v1_heavy','GammaUniform10D_v1_light','GaussianUniform10D_v2','GaussianUniform10D_v2_heavy','GaussianUniform10D_v2_light', 'WeibullGamma_Uniform20D_v1', 'WeibullGammaUniform20D_v1_heavy', 'WeibullGammaUniform20D_v1_light'] + data20D
...
def get_ci(obsT, predT, delta):
    ci_list = []
    for d in range(obsT.shape[1]):
        ci_d = concordance_index(obsT[:, d], predT[:, d], delta[:, d])
        ci_list.append(ci_d)
    ci = np.mean(ci_list)
    # ci = concordance_index(obsT, predT, delta)
    return ci

def get_ci_cens(obsT, predT, delta):
    risk = -predT
    tau = obsT[delta == 1].max()
    obsT_surv = Surv.from_arrays(event=delta, time=obsT)
    mask = obsT < tau
    risk_test = risk[mask]
    obsT_test = obsT_surv[mask]
    ci_cens = concordance_index_censored(obsT_test['event'], obsT_test['time'], risk_test)
    return ci_cens[0]

def get_ci_ipcw(obsT, predT, delta):
    risk = -predT
    tau = obsT[delta == 1].max()
    obsT_surv = Surv.from_arrays(event=delta, time=obsT)
    mask = obsT < tau
    risk_test = risk[mask]
    obsT_test = obsT_surv[mask]
    ci_ipcw = concordance_index_ipcw(obsT_surv, obsT_test, risk_test)
    return ci_ipcw[0]

def get_auc(obsT, predT, delta, tau):
    auc = cumulative_dynamic_auc(obsT, predT, delta, tau)
    return auc

def get_ibs(obsT, predT, delta, tau):
    ibs = integrated_brier_score(obsT, predT, delta, tau)
    return ibs

def get_ql(obsT, predT, tau, u, weights):
    t = np.minimum(obsT, u)
    e = t - predT
    temp = weights * e * ( tau - (np.sign(-e) + 1)/2 )
    # temp = np.maximum(tau * e, (tau - 1) * e)
    ql = np.mean(temp)
    return ql

def get_censdcal(y_preds, y_true, cen_indicator, taus, n_quantiles):
    # now D-calibration for censored data
    # X-CAL: Explicit calibration for survival analysis, eq 9 and 10
    # for each data point, find predicted quantile
    # y_true is the observed time
    diffs = y_preds - np.expand_dims(y_true,axis=1)
    closest_q_idx = np.argmin(np.abs(diffs),axis=1)
    closest_q = []
    for i in range(y_true.shape[0]):
        closest_q.append(taus[closest_q_idx[i]])
    ...
    Dcal_cens = 100*np.sum(np.square(dcal_data_cens[:,0]-dcal_data_cens[:,1]))  # this is CensDCal from paper
    # metrics['Dcal_cens'] = 100*np.sum(np.square(dcal_data_cens[:-1,0]-dcal_data_cens[:-1,1]))
    Dcal_cens_data = dcal_data_cens
    # warning! there is a very slight difference between cens and nocens Dcal because of crossing quantiles!
    # Dcal_nocens is slightly preferred
    CensDCal = Dcal_cens
####
    return Dcal_cens_data, CensDCal

# Added by Shuai
def calculate_mse_surv(test_df, median_surv_time_cox, E):
    """
    ...
    """
    # Convert median_surv_time_cox to a numpy array if it is not already
    median_surv_time_cox_array = median_surv_time_cox.squeeze()

    # Filter out the infinite values
    finite_mask = np.isfinite(median_surv_time_cox_array)

    # Filter the test_df and median_surv_time_cox using the finite_mask
    filtered_E = E.squeeze()[finite_mask]
    filtered_Y = test_df['Y'].squeeze()[finite_mask]
    filtered_median_surv_time_cox = median_surv_time_cox_array[finite_mask]

    # Calculate the MSE and MMSE
    mse = np.mean(np.square(np.log(filtered_Y) - np.log(filtered_median_surv_time_cox)))
    mmse = get_mse(np.log(filtered_Y), np.log(filtered_median_surv_time_cox), filtered_E)
